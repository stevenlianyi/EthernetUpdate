 ...
 * 
 * @file EthernetUpdate.cpp based on HTTPUpdate.cpp
 * @date 2020/12/24
 * @author Steven Lian
 *
...
EthernetUpdate::EthernetUpdate(void)
        : _httpClientTimeout(8000), _ledPin(-1)
{
}

EthernetUpdate::EthernetUpdate(int httpClientTimeout)
...
EthernetUpdateResult EthernetUpdate::update(EthernetClient& client, const String& url, const String& currentVersion)
{
...
EthernetUpdateResult EthernetUpdate::updateSpiffs(EthernetClient& client, const String& url, const String& currentVersion)
{
    EthernetHTTPClient http;
    if(!http.begin(client, url))
    {
...
EthernetUpdateResult EthernetUpdate::update(EthernetClient& client, const String& host, uint16_t port, const String& uri,
 ...
 */
int EthernetUpdate::getLastError(void)
{
    return _lastError;
}
 ...
 * @return String error
 */
String EthernetUpdate::getLastErrorString(void)
{
    ...
    if(_lastError > -100) {
        return String("HTTP error: ") + EthernetHTTPClient::errorToString(_lastError);
    }
...
/**
 *
 * @param http EthernetHTTPClient *
 * @param currentVersion const char *
 * @return EthernetUpdateResult
 */
EthernetUpdateResult EthernetUpdate::handleUpdate(EthernetHTTPClient& http, const String& currentVersion, bool spiffs)
{

    EthernetUpdateResult ret = HTTP_UPDATE_FAILED;
    ...
    //uint8_t ethMacAddress[8];
    
    //http.addHeader("x-ESP32-STA-MAC", WiFi.macAddress()); //Steven Lian modified 
    //http.addHeader("x-ESP32-AP-MAC", WiFi.softAPmacAddress()); //Steven Lian modified 
    ...
    case HTTP_CODE_OK:  ///< OK (Start Update)
        if(len > 0) {
            ...
            if(!startUpdate) {
                ...
                EthernetClient * tcp = http.getStreamPtr();

// To do?                WiFiUDP::stopAll();
// To do?                EthernetClient::stopAllExcept(tcp);
 ...
 */
bool EthernetUpdate::runUpdate(Stream& in, uint32_t size, String md5, int command)
{
